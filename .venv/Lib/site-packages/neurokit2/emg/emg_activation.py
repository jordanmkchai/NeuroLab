...
def emg_activation(
    ...
    * Silva H, Scherer R, Sousa J, Londral A , "Towards improving the ssability of
    ...
    if method == "threshold":
        ...
        if emg_cleaned is None:
            raise ValueError(
                "NeuroKit error: emg_activation(): 'biosppy' method needs 'emg_cleaned' (cleaned EMG) signal to be passed."
            )
        if size is None:
            size = 0.05
        ...
        if emg_cleaned is None:
            raise ValueError(
                "NeuroKit error: emg_activation(): 'silva' method needs 'emg_cleaned' (cleaned EMG) signal to be passed."
            )
        if size is None:
            size = 20
...
def _emg_activation_threshold(emg_amplitude, threshold="default"):
    ...
    if threshold > np.max(emg_amplitude):
        raise ValueError(
            "NeuroKit error: emg_activation(): the threshold specified exceeds the maximum of the signalamplitude."
        )
...
def _emg_activation_silva(emg_cleaned, size=20, threshold_size=22, threshold="default"):
    ...
    for k in range(0, len(threshold_mvgav)):
        ...
        elif tf_mvgav[k] >= threshold_mvgav[k] and tf_mvgav[k] >= threshold:
