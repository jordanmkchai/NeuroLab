...
def complexity_tolerance(signal, method="maxApEn", r_range=None, delay=None, dimension=None, show=False):
    ...
    * Singh, A., Saini, B. S., & Singh, D. (2016). An alternative approach to approximate entropy
    ...
    if method in ["traditional", "sd", "std", "default"]:
        ...
        n = len(signal)
        r = np.std(signal, ddof=1) * (0.2811 * (dimension - 1) + 0.0049 * np.log(n) - 0.02 * ((dimension - 1) * np.log(n)))
...
def _optimize_tolerance_neighbours(signal, r_range=None, delay=None, dimension=None):
    ...
    counts = np.array(
        [np.mean(kdtree.query_radius(embedded, r, count_only=True).astype(np.float64) / embedded.shape[0]) for r in r_range]
    )
    # Closest to 0.02 (2%)
